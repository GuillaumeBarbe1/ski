...
public class DaoPisteImpl extends JpaBaseDao<Piste, Serializable> implements DaoPiste {
    ...
    @Override
    public Iterable<Piste> findAllByStation(Long id) throws DaoException {
        try {
            return this.em.createQuery("SELECT p FROM Piste p WHERE p IN (SELECT p FROM Station s LEFT JOIN s.pistes p WHERE s.id = :id)", Piste.class)
                    .setParameter("id", id)
                    .getResultList();
        } catch (Exception e) {
    ...
    @Override
    public Iterable<Piste> findAllByForfait(Long id) throws DaoException {
        try {
            return this.em.createQuery("SELECT p FROM Piste p WHERE p IN (SELECT p FROM Forfait f LEFT JOIN f.pistes p WHERE f.id = :id)", Piste.class)
                    .setParameter("id", id)
                    .getResultList();
        } catch (Exception e) {
            throw new DaoException(e);
